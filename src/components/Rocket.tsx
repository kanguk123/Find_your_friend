...
export default function Rocket() {
    ...
    function spawnAtEarth() {
        ...
        g.position.copy(r.clone().add(n.clone().multiplyScalar(earthR + START_ALT)));
        ...
        const q = new Quaternion().setFromUnitVectors(new Vector3(0, 0, -1), forward);
    ...
    useEffect(() => { if (!spawned) spawnAtEarth(); /* eslint-disable-next-line */ }, [bodyPositions, spawned]);
    useEffect(() => { setSpawned(false); }, [requestReset]);
    useEffect(() => { if (!followRocket) vel.current = 0; }, [followRocket]);

    function updateInputs(dt: number) {
        let thrust = 0;
        let anyInput = false;

        if (key.down("KeyW")) { thrust += THRUST_ACCEL; anyInput = true; }
        if (key.down("KeyS")) { thrust -= THRUST_ACCEL; anyInput = true; }

        boosting.current = key.down("Space") && (thrust >= 0 || vel.current >= 0);
        anyInput = anyInput || key.down("Space");

        if (key.down("ArrowUp"))   { euler.x += PITCH_RATE * dt; anyInput = true; }
        if (key.down("ArrowDown")) { euler.x -= PITCH_RATE * dt; anyInput = true; }

        const yawIn = (key.down("KeyA") ? -1 : 0) + (key.down("KeyD") ? 1 : 0);
        if (yawIn !== 0) { euler.y -= yawIn * YAW_RATE * dt; anyInput = true; }

        const rollIn = (key.down("KeyQ") ? -1 : 0) + (key.down("KeyE") ? 1 : 0);
        if (rollIn !== 0) { euler.z += rollIn * ROLL_RATE * dt; anyInput = true; }
        else {
        ...
        vel.current = Math.sign(vel.current) * Math.max(0, Math.abs(vel.current) - DRAG_K * Math.abs(vel.current) * dt);
    ...
    function updateKinematics(dt: number) {
        ...
        if (followRocket) {
            ...
            camera.lookAt(g.position.clone().add(fwd.clone().multiplyScalar(TPV_LOOK_AHEAD)));
        }
    }
    ...
    useFrame((_, dt) => {
        ...
        if (!followRocket) {
            ...
            // 입력을 누르면 바로 추적으로 전환
            if (
                key.down("KeyW") || key.down("KeyA") || key.down("KeyS") || key.down("KeyD") ||
                key.down("ArrowUp") || key.down("ArrowDown") || key.down("ArrowLeft") || key.down("ArrowRight") ||
                key.down("KeyQ") || key.down("KeyE") || key.down("Space")
            ) {
                setFollowRocket(true);
                setFlyToTarget(undefined);
            }
            ...
            <group ref={physRef} visible={visible}>
                ...
                <group rotation={visualRotation}>
                    ...
                    <mesh position={[0, (BODY_H - NOSE_H) * 0.5, 0]}>
                        ...
                        <meshStandardMaterial color="#e9eef2" roughness={0.35} metalness={0.15} />
                    </mesh>
                    {/* 노즈 */}
                    <mesh position={[0, BODY_H - NOSE_H * 0.5, 0]}>
                        <coneGeometry args={[BODY_R * 0.93, NOSE_H, 24]} />
                        <meshStandardMaterial color="#f3f6f8" roughness={0.28} metalness={0.15} />
                    </mesh>
                    {/* 인터스테이지(띠) */}
                    <mesh position={[0, BODY_H * 0.42, 0]}>
                        <cylinderGeometry args={[BODY_R * 1.02, BODY_R * 1.02, BODY_R * 0.22, 24]} />
